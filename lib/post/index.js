...
module.exports = function(opts) {
  ...
  (function(proto) {
    ...
    // post.prototype.setPreview
    proto.setPreview = function(text) {
      this[sym].preview = utils.stringify(text);
      return this;
    };

    // post.prototype.setText
    proto.setText = function(text, preview) {
      preview = (preview === true) ? true : false;
      text = utils.stringify(text);

      if (!this[sym].preview || preview) {
        let fragments = text.split('\n\n');

        this.setPreview(fragments.shift());
